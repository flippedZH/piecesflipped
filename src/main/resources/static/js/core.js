...
function calcUpQi(){
        //提子记录为零  //
    if(record.length===0||jumpPointer<0||jumpPointer>record.length-1){
        return false;
        }
    blackUpCount=0;
    whiteUpCount=0;
...
//Play 棋局逻辑  player输入为：lockQiRole 在页面中定义
function play(row, col,player=undefined) {
    //如果player不为空
    if(player&&(player===qiType.black||player===qiType.white)){
        whoIsPlay=player;  //默认为 黑先
    }
    console.log("row,col:",row,col);
    //判断坐标是否在棋盘内，回调函数
    onTheBoard(row,col,function (){exception("超出棋盘限制")});

    //处理已有棋子在此
    if (pan[row][col] !== 0) {exception("此数有棋子")}

    let qi=[];
    let deadArray=[];

    //判断是否有气  
    if(haveAir(row,col,qi)===0){//气为零但时发生吃子的情况
        let is_eat;
        //是否能吃 对手的棋
    ...
    //根据deadArray判断提子数  谁在下 就算是谁的提子
    addRecord([row,col,whoIsPlay,deadArray]);

    if(whoIsPlay===qiType.white){
        whiteUpCount+=deadArray.length; //在原来基础上面加
    }else{
        blackUpCount+=deadArray.length;
    }

    setAreaQi(deadArray); //清空死棋子块为空
    nextPlay(); //角色互换
}
...
function eat(row,col,qi) {
    //死亡区域
    let deadRound=[];
    for(let i=0;i<qi.length;i++){ //处理周围的气数
        //先处理对手的
        if(qi[i][2]!==whoIsPlay){
            //气点与势力范围
            let round=[],point=[];
...
function haveAir(row,col,array,onlyEmpty=false){
    ...
    function isAir(r, c) {
        let flag;
        //四个方向判断   这里回调函数不报错 
        flag=onTheBoard(r,c,function () {return false;});
        //如果在棋盘中
        if(flag){
            if(onlyEmpty){
                //之前没有棋子
                ...
                //无重复添加：在判断气的过程中，需要将周围的棋子添加到array中？
                //会有需要重复判断的元素
                arrayAdd(r,c,pan[r][c],array)
            }
            if(pan[r][c]===qiType.empty){
